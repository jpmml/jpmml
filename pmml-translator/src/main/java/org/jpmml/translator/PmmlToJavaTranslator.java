...
public class PmmlToJavaTranslator {
    ...
    static public Class<?> createModelClass(String className, String packageName, String javaSource, List<String> compilerOptions) throws Exception {
        ...
        if (status) {
        	...
        	URLClassLoader classLoader = URLClassLoader.newInstance(new URL[] { classPathFile.toURI().toURL() });
      ...
      *             if trouble writing class files.
      */
     static private boolean compile(Writer writer, List<String> compilerOptions, JavaFileObject... source) {
